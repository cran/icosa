---
title: "The R package 'icosa' v0.9 for global triangular and hexagonal gridding"
...
the grid object and the data can easilly be converted to a ’\verb@SpatialPolygonsDataFrame@’ class.

```{r spdf, rgl=TRUE,dev='png',dpi=300}
...
```{r tri25, rgl=TRUE,dev='png',dpi=300,}
# the facelayer of the occupied cells
gHigh<-trigrid(c(8,8))
...
As the core algorithm which is shared with the function ’\verb@locate()@’ operates in 3d space, all these lookups are carried out by invoking a very simple conversion function that translate the ’\verb@sp@’ class objects to their 3d counterparts (i.e. ’\verb@SpatialLines3d@’, ’\verb@SpatialPolygons3d@’) that are defined in the ’\verb@icosa@’ package. This allows 3d plotting if the ’\verb@lines3d()@’ function is invoked on the original calls (no polygon plotting method is defined until this point).

```{r tri25b, rgl=TRUE,dev='png',dpi=300,}
# plot the faces
...
The ’\verb@resample()@’ function can also be used to upscale, or downscale a ’\verb@facelayer@’ linked to ’\verb@trigrid@’ or ’\verb@hexagrid@’ object as well. 

```{r tri31b, plot=TRUE,dev='png',dpi=300,}
...
Dowscaling naturally decreases the variance of the values. Currently there is only one method for downscaling and one method for upscaling data stored in a ’\verb@facelayer@’ class but more will be implemented in the future. 
...
Random walk simulations can also be built using the graph represetation. In this example a random walker will walk 1000 steps on the grid, starting from face ’\verb@F6284@’. 
